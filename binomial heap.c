...
int bin_LINK(struct node* y, struct node* z) { 
	y->parent = z;
	y->sibling = z->child; z->child = y;
	z->degree = z->degree + 1;
}
struct node* CREATE_NODE(int k) { 
	struct node* p;//new node;
	p = (struct node*) malloc(sizeof(struct node)); p->n = k;
	return p;
}
			...
			x = next_x;
				...
				if (x->n <= next_x->n) {
					...
					if (prev_x == NULL) 
						...
						prev_x->sibling = next_x; bin_LINK(x, next_x);
					x = next_x;
				}
			}
		next_x = x->sibling;
...
struct node* bin_HEAP_MERGE(struct node* H1, struct node* H2) { 
	...
	while (y != NULL && z != NULL)
	{
		...
		else if (y->dgree == z->degree) { 
			a = y->sibling;
			y->sibling = z; y = a;
		} else {
			b = z->sibling; z->sibling = y; z = b;
		}
	}
return H;
}
			...
			switch (l) 
			{
				case 1:
						do {
								...
								printf("NOW THE HEAP IS:"); DISPLAY(H);
				...
				case 2:
						do {
								printf("EXTRACTING THE MINIMUM KEY NODE"); p = bin_HEAP_EXTRACT_MIN(H);
								if (p != NULL)
